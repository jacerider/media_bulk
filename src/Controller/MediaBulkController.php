...
namespace Drupal\media_bulk\Controller;
 ...
 */
class MediaBulkController extends ControllerBase {
  ...
  public function listUpload() {
    $supported_media_types = $this->config('media_bulk.settings')
      ->get('media_types');
    ...
    foreach ($media_types as $id => $media_type) {
      ...
      $url = Url::fromRoute('media_library.bulk.upload_form', ['media_type' => $id]);
  ...
  public function uploadForm(MediaTypeInterface $media_type) {
    ...
    \Drupal::request()->query->set('media_library_view_hide', TRUE);

    $ui = \Drupal::service('media_library.ui_builder')->buildUi($state);
    $ui['#attached']['library'][] = 'media_bulk/library';
